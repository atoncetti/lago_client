defmodule LagoApiClient.Config do
  ...
  @default_stack [
    {LagoApiClient.Plugin.JasonSerializer, :encode_body, []},
    {LagoApiClient.Plugin.HTTPoisonClient, :request, []},
    {LagoApiClient.Plugin.JasonSerializer, :decode_body, []},
    {LagoApiClient.Plugin.TypedDecoder, :decode_response, []},
    {LagoApiClient.Plugin.TypedDecoder, :normalize_errors, []}
  ]
  ...
  > useful for plugins. See `LagoApiClient.Plugin` for more information.
  ...
  argument. Configuration passed in this way always takes precedence over global configuration.
      ...
      # Local options:
      LagoApiClient.Customers.create_customer(%CustomerCreateInput{}, server: "https://gh.example.com")

      # Application environment (ex. config/config.exs):
      config :lago_api_client, server: "https://gh.example.com"

  ## Options

  The following configuration is available using **local options**:

    * `server` (URL): API server to use. Useful if the client would like to target a LagoApiClient
    ...
    * `stack` (list of plugins): Plugins to control the execution of client requests. See
      `LagoApiClient.Plugin` for more information. Defaults to the default stack below.
    ...
    * `wrap` (boolean): Whether to wrap the results of the API call in a tagged tuple. When
      ...
      **Note**: Unwrapped responses violate the type specifications provided for each client
      operation. To avoid Dialyzer errors, consider using `LagoApiClient.raw/4` instead.
  ...
  The following configuration is available using the **application environment**:
    ...
    * `apps` (list of tuples): LagoApiClient App configurations. Each app config is a 3-tuple containing
      a name (atom), app ID (integer), and private key (string, PEM format). See `LagoApiClient.app/1`
      for more information.

    * `default_auth` (`t:LagoApiClient.Auth.auth/0`): Default API authentication credentials to use when
    ...
    * `webhook_secret` (string): Secret value provided to LagoApiClient for signing webhook requests.
      See `LagoApiClient.Webhook` for more information. Defaults to no signature verification.

  ## Plugins

  Client requests are implemented using a series of plugins. Each plugin accepts a
  `LagoApiClient.Operation` struct and returns either a modified operation or an error. The collection of
  ...
  > #### Warning {:.warning}
  >
  > Stack entries without options, like `{LagoApiClient.Plugin.TestClient, :request}`, look like keyword
  ...
  @doc """
  Get the configuration of a LagoApiClient App by its name
  ...
  def app(app_name) do
    case Application.fetch_env(:lago_api_client, :apps) do
  ...
  def app_name do
    Application.get_env(:lago_api_client, :app_name)
  end
  ...
  """
  @spec default_auth :: LagoApiClient.Auth.auth()
  def default_auth do
    Application.get_env(:lago_api_client, :auth)
  end
  ...
  the response will fall back to the application environment given with the following form:
      ...
      config :lago_api_client, MyPlugin, some: :option
  ...
  def plugin_config(config \\ [], plugin, key, default) do
    if value = Keyword.get(config, key) do
      ...
      Application.get_env(:lago_api_client, plugin, [])
  ...
  defp config(config, key, default) do
    Keyword.get(config, key, Application.get_env(:lago_api_client, key, default))
  end
end
